...
class MKHelp(WealthHelp):
    def __init__(self) -> None:
        ...
        trans_opts.add_action(
            ...
            "--output",
            metavar="DIR",
            description=_("指定目标文件夹，如果不指定则默认为当前工作目录。"),
        )
        trans_opts.add_action(
            "--sort",
        ...
        trans_opts.add_action(
            ...
            "--overwrite",
            description=_(
                "启用[red bold]强制覆盖模式[/]后，将忽略配置文件中定义的覆盖选项。"
            ),
        )
        trans_opts.add_action(
            "-n",
            "--no-overwrite",
            description=_(
                "启用[green bold]安全模式[/]之后，将忽略一切覆盖选项，无论如何也不会覆盖目标文件。"
            ),
        )

        basic_opts = self.add_group(_("其它操作"), _("运行除了转码之外的操作"))
        ...
        basic_opts.add_action(
            ...
            "--save",
            metavar="FILE",
            description=_("保存转码任务为脚本文件，将不再主动进行转码操作。"),
        )
        basic_opts.add_action(
            "-c",
            "--continue",
            description=_(
                "加载上次的[u]所有[/]转码任务并重新执行。\n如果想要继续运行未完成的任务，建议附加 -n 选项。"
            ),
        )
