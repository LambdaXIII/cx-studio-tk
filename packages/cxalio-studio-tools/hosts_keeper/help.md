...
**假装模式（`--pretend` / `-p`）的行为细节：**

假装模式下，程序仍然完整执行配置解析、内容获取、hosts 组装的全流程，但：

- 每个内容块（contenter）处理前会模拟 **4 秒** 的处理延迟，用于观察进度条的动态变化
- 不会实际修改 hosts 文件，最终内容输出到标准输出
- 适用于预览更新效果、验证配置是否正确

**输出顺序与查重：**

- **输出顺序**：用户自定义内容（未被标记包裹的区域）最前 → 启用的配置按 `priority` 降序（数值大者在前，相同优先级按配置文件被发现顺序）→ 配置内按内容源书写顺序
- **域名冲突**：同一域名出现多次时，**先出现者生效**（与系统解析行为一致——Linux/Windows 均以首个匹配生效）；后出现者以 `# ` 注释形式保留在自身配置块内
- 注释保留的条目是**生成产物**：冲突消失后（如禁用高优先级配置）下一次更新自动恢复为有效条目，无需手动处理
...
hostskeeper 使用 `##### <id> START #####` 与 `##### <id> END #####` 标记配置块
（行首行尾各五个 `#`，夹大写 `START` / `END`，`<id>` 为配置文件 ID）。这是
hostskeeper 的**保留标记格式**，请务必了解其行为：

- 命中该格式即按配置块处理：两个标记之间的内容属于该配置的领地，更新重建时
  **整体覆盖——块内手动添加的内容会被删除**
- **不要**使用五个 `#` 夹大写 START/END 做装饰性分区（如
  `##### 以下为手动维护区域 START #####`）——一旦命中格式即被当作配置块处理，
  块内内容会被覆盖。无意撞上该格式属于操作者责任，程序不会自动修复
- 更新时若发现**不配对**的标记（有 START 无 END / 有 END 无 START），程序会报告
  hosts 结构异常并提示手动检查，但不会自动修复
- 更新时若发现标记对应的配置文件已删除或改名，程序会报告**残留标记块已清除**
...
1. **用户自定义内容最先输出**，其中的域名**不会被任何配置覆盖**（自定义内容优先）
2. **优先级高的配置先输出**（priority 数值越大越靠前；相同优先级按配置文件被发现顺序，顺序不确定）
3. **域名冲突时先出现者生效**，后出现者以 `# ` 注释保留在自身配置块内（冲突消失后自动恢复为有效条目）

```
原有 hosts 自由区域（自定义内容，域名受保护）
------------------------------
[google START]                 # 高优先级配置
Google 相关记录...
[google END]
...
- 每个配置文件的详细信息
- 内容处理进度
- 最终生成的 hosts 内容（完整文本输出到**标准错误**，不影响标准输出的正式输出；
  与 `-p` 同时使用时，标准输出与标准错误各有一份全文）
...
4. **合理优先级**：常用配置设高优先级，保证其条目先出现生效，避免被低优先级配置的冲突条目抢占
