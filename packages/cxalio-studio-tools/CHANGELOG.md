...
- 领域文档迁移至仓库根级单套：cxalio 的 `CONTEXT.md` 与 `docs/adr/` 移出本包，合并入根 `CONTEXT.md`（cxalio Domain 分区）与根 `docs/adr/` 单一编号序列；本包 ADR 原 0001-0011 重编号 0011-0021（旧→新映射见根 `docs/adr/0022`）
- 代码 docstring 剥离 ADR 引用（`cx_note/common`）——docstring 独立呈现，不引用仓库外部文档（规范入根 AGENTS.md）
...
- **新包 `cx_note`**（第 6 个工具，`__version__` 初始 1.0.0）：终端快速便签——条目 = 内容 + 三态（待办/正在做/已完成）+ 日期，归属「域」字面命名空间（ADR-0019），4 位小写 base36 ID 全局唯一终身不变
...
- **渲染**（列表展示重设计，ADR-0020）：平铺域块——域标题行（`cx.note.section`）外置 + 每域独立无框三列小 Table（标号自然宽 / 内容 ratio=1、含换行按 Markdown 渲染 / ID 徽章 width=4 右对齐贴行尾），块间空行分隔；`cx.note.*` 样式经 push_theme 叠加。初版整表分节行断行割裂回退逐行、`█` 树状符号与徽章灰底均在本批验收返工中废弃
...
- **注册**：`pyproject.toml` scripts/wheel packages/mo include、`babel.cfg` 提取域；CONTEXT.md 词汇与 ADR-0019 随本批入库
